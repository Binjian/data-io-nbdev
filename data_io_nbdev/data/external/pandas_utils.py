...
def decode_mongo_records(
    ...
    dict_observations_list = [  # list of observations as dict with tuple key suitable as MultiIndex
        {
            (
                meta["episodestart"],
                meta["vehicle"],
...
def decode_episode_batch_to_padded_arrays(
    ...
    states_list = [
        episodes.loc[
            idx[i, :, :, :, :], idx["state", ["velocity", "thrust", "brake"]]
    ...
    actions_list = [
        episodes.loc[
            idx[i, :, :, :, :], idx["action", torque_table_row_names]
        ].values.tolist()  # type: ignore
        for i in episodes.index.get_level_values(0)
    ]  # type: ignore
    ...
    nstates_list = [
        episodes.loc[
            idx[i, :, :, :, :], idx["nstate", ["velocity", "thrust", "brake"]]
        ].values.tolist()  # type: ignore
        for i in episodes.index.get_level_values(0)
    ]  # type: ignore
